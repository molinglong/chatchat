    ...
    >
      ...
      {expanded && (
        <div className="px-3.5 py-3 text-xs leading-relaxed">
          <MarkdownRenderer content={content} messageId="summary-card" />
        </div>
      )}
    </div>
  )
...
function MessageBubbleInner({
  ...
  /** HTML 复制源:挂载一个离屏 MarkdownRenderer,按需读取 innerHTML */
  ...
  const handleCopyHtml = useCallback(() => {
    ...
    if (!htmlMirrorRef.current) {
      toast.error('HTML 渲染尚未就绪,请稍后再试', { title: '复制失败' })
      return
    }
    const html = htmlMirrorRef.current.innerHTML
    if (!html || !html.trim()) {
  ...
  className="copy-menu absolute z-30 top-full mt-1 right-0 sm:left-0 min-w-[140px] rounded-lg border border-line/60 bg-surface shadow-xl py-1 text-xs
      ...
      {/* HTML 复制源 —— 离屏 MarkdownRenderer.
          visibility:hidden + 绝对定位到屏幕外,确保 ref 可读 innerHTML 但不显示.
          pointer-events:none 避免意外拦截. */}
      {showActions && bodyText && (
        <div
          ref={htmlMirrorRef}
          aria-hidden
          style={{
        ...
        >
          <MarkdownRenderer content={bodyText} messageId={message.id} />
        </div>
      )}
    </div>
  )
